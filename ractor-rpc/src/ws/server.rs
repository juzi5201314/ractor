...
use crate::{deserialize, MessageHeader, serialize};
...
impl Server {
    ...
    pub async fn run(self) {
        while let Ok((stream, _peer_addr)) = self.tcp_listener.accept().await {
            let stream =
                match tokio_tungstenite::accept_async_with_config(stream, Some(self.config.clone()))
                    .await {
                    Ok(s) => s,
                    Err(err) => {
        ...
        while let Some(Ok(msg)) = incoming.next().await {
            match msg {
                Message::Binary(msg) => {
                    ...
                    match register.get(&header.identity_id) {
                        None => continue,
                        Some(f) => outgoing
                            .send(Message::Binary(serialize(&crate::Message {
                                identity_id: 0,
                                unique_id: header.unique_id,
                                payload: f(&header.payload).await.unwrap()
                            }).unwrap()))
                            .await
